# -*- coding: utf-8 -*-
"""
EEG Wavefield Viewer — CUDA Moiré (local Δk→λ overlay + 3D stack)

What’s new vs. your CUDA build:
  • Live Δk→λ overlay: estimate local wavevectors from band-A and band-B phase
    fields and display λ_local(x,y) = 2π / |∇φ_A - ∇φ_B|.
  • Title readout: Δf, predicted λ ≈ c/Δf, and “cells across diameter”.
  • Button: toggle overlay   • Slider: overlay opacity
  • Keeps your GPU-accelerated 3D Moiré stack (CuPy if available).

Usage:
  python eeg_wavefield_viewer_cuda_moire_local.py -f your.edf --band 3 10 --mode huygens --fps 30

Tips:
  • Click “Δk→λ” to show/hide the overlay; use “Overlay α” slider to adjust opacity.
  • Press “3D Stack” or key “3” to open the GPU 3D frequency stack viewer.
"""
...
# ----------------------------
def main():
    p = argparse.ArgumentParser(description="Interactive EEG wavefield viewer with CUDA-accelerated moiré tools")
    ...
    print("  L toggle Δk→λ overlay   3 open 3D Stack")
    viewer.show()
